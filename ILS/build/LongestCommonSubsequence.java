...
public class LongestCommonSubsequence {
    ...
    public LongestCommonSubsequence(int lx, int ly) {
        ...
        LCS = new SubInstance(lx, ly);
    }
    
    private void fillMatrix(SubInstance S){
        setShiftMap(S);
    ...
    private SubInstance retrieveSolution(SubInstance S){
        LCS.init();
        int n = subDim[X], m = subDim[Y];
        int i = n, j = m;
    ...
    public SubInstance getLCS(SubInstance S){
        fillMatrix(S);
        return retrieveSolution(S);        
    }
    
    private void setShiftMap(SubInstance S){
        int curr = 0, jump;
