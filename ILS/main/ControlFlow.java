...
public class ControlFlow{
    ...
    private final IteratedLocalSearch procedure;
    
    public ControlFlow(IraceManager im, Parser p) throws FileNotFoundException{
        Instance I = p.getInstance(im.getInstancePath());
        
        Parameters P = new Parameters().setDistribution(im.getDistribution())
                .setBias(im.getBias()).setDeleteFactor(im.getDeleteFactor())
                .setTimeLimit(this.getTimeLimit(I)).setSeed(im.getSeed());
        
        ProcedureFactory pf = new ProcedureFactory(I, P);
        procedure = pf.makeILS();
    }
