...
public class ControlFlow{
    ...
    public ControlFlow(IraceManager im, Parser p) throws FileNotFoundException{
        ...
        Parameters P = new Parameters().setDistribution(im.getDistribution())
                .setBias(im.getBias()).setTimeLimit(this.getTimeLimit(I));
        
        ProcedureFactory pf = new ProcedureFactory(I, P);
        procedure = pf.makeGRASP();
    }
    ...
    private long getTimeLimit(Instance I){
        float x = 2*(I.getX().length + I.getY().length - I.getAlphabetEncoding());
        return (long)(x);
    }
}